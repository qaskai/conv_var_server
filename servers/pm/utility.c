...
#include <string.h>
/* INFORM MUTEX SERVER */
static int find_cv_number(endpoint_t* val) {
	int i;
	for (i=0; i<NR_PROCS; ++i)
		if (!strcmp(mproc[i].mp_name, "cv"))
			break;
	if (i==NR_PROCS)
		return -1;
	else {
		*val = mproc[i].mp_endpoint;
		return 0;
	}

}

void tell_mutex(endpoint_t proc, int call) {
    if (call!=5 && call!=6) // pm send only those messages
        return;
    message m;
    m.m_type = call;
    m.m1_i1 = proc;
    endpoint_t cv;
    if (find_cv_number(&cv)) // no cv found
        return;
    if ( asynsend3(cv, &m, AMF_NOREPLY) != OK )
        fprintf(stderr, "Informing cv from PM failed\n");
}